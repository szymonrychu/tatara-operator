...
type ProjectSpec struct {
	...
	// (marked <!-- tatara-proposed-by:<kind> content-sha256:<hash> -->) straight
	// into implement->review->auto-merge->deploy without a second human gate: the
	...
	// (the marker's content fingerprint must still match).
	//
