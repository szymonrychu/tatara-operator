...
type TaskStatus struct {
	...
	//
	// stage.ArmedClock's podless WORK clock, operator_task_stage_age_seconds and
	// operator_merge_cursor_stalled_seconds all read this back (added to
	// now.Sub(StageEnteredAt)) so the re-entered clock reads CONTINUOUS elapsed
	// time rather than a sawtooth that resets every re-entry - which also means
	// the very next reconcile pass after an un-park catches a still-red MR
	// instead of waiting out another full budget, collapsing the ~16h worst case
	// down to roughly one budget plus a few reconcile ticks.
	// +optional
