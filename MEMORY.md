...
- 2026-06-07 (N1) MEMORY_BASE_URL removed from config.Config and operator config; per-Project endpoint (status.memory.endpoint) replaces it in N3. ingest.Config.MemoryBaseURL retained (N3 repoints it to project.Status.Memory.Endpoint). wire.go ingestConfigFromConfig no longer propagates the global URL; wire_test.go updated to expect empty MemoryBaseURL from the mapping.
...
- 2026-06-07 (N2) client.Apply (Patch variant) marked //nolint:staticcheck; migration to typed r.Apply(ctx, applyconfig) requires generated applyconfiguration types for all 10 stack objects - deferred to N4. The Patch variant is functionally identical and the deprecation notice states it will not be removed before v0.30.
