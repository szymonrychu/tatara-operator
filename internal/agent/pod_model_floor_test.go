...
func TestModelForKindOnRepo_HelmfileFloor(t *testing.T) {
	...
	cases := []struct {
		...
		{"review floored on helmfile", "review", "", "tatara-helmfile", "claude-opus-4-8"},
		// A broken override for implement must not break the fix flow.
		{"broken implement floored on helmfile", "implement", "", "tatara-helmfile", "claude-opus-4-8"},
		...
		{"healthCheck exempt on helmfile", "brainstorm", "healthCheck", "tatara-helmfile", "claude-opus-4-8"},
	}
...
func TestBuildPod_HelmfileReviewFloored(t *testing.T) {
	...
	if got := mk(helmfileRepo); got != "claude-opus-4-8" {
		t.Fatalf("BuildPod MODEL for review on tatara-helmfile = %q, want claude-opus-4-8 (floored)", got)
	}
