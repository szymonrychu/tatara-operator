...
// Ingress builds the per-Project Ingress exposing this project's memory on
// cfg.IngressHost under a project-scoped path with a rewrite that strips the
// prefix. Returns nil when cfg.IngressHost is empty (external exposure
// disabled). Owner-ref'd to the Project; no TLS block (the host cert is owned by
// the operator Ingress) and no nginx auth annotations (memory enforces OIDC at
// the app).
