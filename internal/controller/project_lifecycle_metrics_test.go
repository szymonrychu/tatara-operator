...
// gatherLifecycleState reads tatara_lifecycle_state{state=state} from reg.
func gatherLifecycleState(t *testing.T, reg *prometheus.Registry, state string) float64 {
	t.Helper()
	mfs, err := reg.Gather()
	if err != nil {
...
// mkLifecycleStateTask creates an issueLifecycle Task and sets its
// Status.LifecycleState.
func mkLifecycleStateTask(t *testing.T, ctx context.Context, name, state string) {
	t.Helper()
...
// TestUpdateLifecycleStateCounts_RecomputeCorrectsSkewedGauge asserts that the
...
func TestUpdateLifecycleStateCounts_RecomputeCorrectsSkewedGauge(t *testing.T) {
	...
	mkLifecycleStateTask(t, ctx, "lc-rc-conv-1", "Conversation")
	mkLifecycleStateTask(t, ctx, "lc-rc-conv-2", "Conversation")
	mkLifecycleStateTask(t, ctx, "lc-rc-merge-1", "Merge")
	...
	other.Status.LifecycleState = "Merge" // should be ignored: Kind != issueLifecycle
	...
	for _, s := range lifecycleStates {
		truth[s] = gatherLifecycleState(t, reg, s)
	}
	...
	r.LifecycleMetrics.SetLifecycleState("Conversation", truth["Conversation"]+17)
	r.LifecycleMetrics.SetLifecycleState("Merge", -5)
	r.LifecycleMetrics.SetLifecycleState("Parked", truth["Parked"]+9)

	// Recompute must restore every state to truth, including zeros.
	r.updateLifecycleStateCounts(ctx)
	for _, s := range lifecycleStates {
		if got := gatherLifecycleState(t, reg, s); got != truth[s] {
