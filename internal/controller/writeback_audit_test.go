...
// --- Finding 3: mergeAllowed uses pre-fetched PRState ---

// countingGetPRStateWriter counts GetPRState calls.
type countingGetPRStateWriter struct {
	scm.SCMWriter
	mu          sync.Mutex
	getPRCalls  int
	state       scm.PRState
...
func (f *countingGetPRStateWriter) GetPRState(_ context.Context, _, _ string, _ int) (scm.PRState, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.getPRCalls++
	return f.state, nil
}
func (f *countingGetPRStateWriter) Merge(_ context.Context, _, _ string, n int, _ string) (string, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.mergeCalled = true
	f.mergeNumber = n
...
func (f *countingGetPRStateWriter) ClosePR(_ context.Context, _, _ string, _ int, _ string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.closeCalled = true
	return nil
}
func (f *countingGetPRStateWriter) Comment(_ context.Context, _, _, _ string) error { return nil }
func (f *countingGetPRStateWriter) OpenChange(_ context.Context, _, _, _, _, _, _ string) (string, error) {
	return "", nil
}

// TestWriteBackSelfImprove_GetPRStateCalledOnce verifies finding 3: the merge
// path must call GetPRState exactly once (authorship gate), not twice.
func TestWriteBackSelfImprove_GetPRStateCalledOnce(t *testing.T) {
	fw := &countingGetPRStateWriter{
		state: scm.PRState{Author: "tatara-bot", CIStatus: "success"},
	}
	r := &TaskReconciler{
		Client:  k8sClient,
		Scheme:  k8sClient.Scheme(),
	...
	task := seedWritebackKindTask(t, "audit-si-once", "audit-si-proj", "audit-si-repo", "audit-si-scm",
		tatarav1alpha1.TaskSpec{
			Goal: "self-improve",
			Kind: "selfImprove",
			Source: &tatarav1alpha1.TaskSource{
				Provider: "github", IssueRef: "o/r#40", IsPR: true, Number: 40,
			},
		},
