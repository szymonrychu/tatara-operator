...
// --- Finding 4: already-merged PR not mis-classified as MergeConflict ---

// mergedPRFakeWriter simulates an already-merged PR: Merge returns ErrMergeConflict
// (GitHub 405), but GetPRState shows Merged=true.
type mergedPRFakeWriter struct {
	scm.SCMWriter
	mu         sync.Mutex
	mergeCalls int
	getPRCalls int
...
func (f *mergedPRFakeWriter) GetPRState(_ context.Context, _, _ string, _ int) (scm.PRState, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.getPRCalls++
...
func (f *mergedPRFakeWriter) Merge(_ context.Context, _, _ string, _ int, _ string) (string, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.mergeCalls++
	return "", f.mergeErr
...
func (f *mergedPRFakeWriter) Comment(_ context.Context, _, _, _ string) error { return nil }
func (f *mergedPRFakeWriter) ClosePR(_ context.Context, _, _ string, _ int, _ string) error {
	return nil
}

// TestWriteBackSelfImprove_AlreadyMergedNotConflict verifies finding 4: when
// Merge returns ErrMergeConflict but GetPRState shows Merged=true, the controller
// must treat it as a successful merge (PROutcomeApplied) not as a conflict
// (MergeConflict re-triage), preventing a completed merge from being mis-labelled.
func TestWriteBackSelfImprove_AlreadyMergedNotConflict(t *testing.T) {
	fw := &mergedPRFakeWriter{
		mergeErr: scm.ErrMergeConflict,
		prState:  scm.PRState{Author: "tatara-bot", Merged: true},
	}
	r := &TaskReconciler{
		Client:  k8sClient,
		Scheme:  k8sClient.Scheme(),
	...
	task := seedWritebackKindTask(t, "si-already-merged", "si-am-proj", "si-am-repo", "si-am-scm",
		tatarav1alpha1.TaskSpec{
			Goal: "self-improve",
			Kind: "selfImprove",
			Source: &tatarav1alpha1.TaskSource{
				Provider: "github", IsPR: true, Number: 77,
			},
		},
	...
	// Must NOT be MergeConflict; must be PROutcomeApplied (success).
	require.NotEqual(t, "MergeConflict", cond.Reason,
		"already-merged PR must not be classified as MergeConflict")
	require.Equal(t, "PROutcomeApplied", cond.Reason,
		"already-merged PR must be treated as PROutcomeApplied")
}
...
// --- Finding 6: close path clears WritebackPending after state change ---

// closeRepostFakeWriter counts ClosePR calls and fails the first one's
// comment-side (ClosePR itself is idempotent in real impl, but we want to
// ensure no second call happens on requeue).
type closeCountFakeWriter struct {
	scm.SCMWriter
	mu          sync.Mutex
	closeCalls  int
	commentBody string
...
func (f *closeCountFakeWriter) ClosePR(_ context.Context, _, _ string, _ int, reason string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.closeCalls++
	f.commentBody = reason
	return nil
}
func (f *closeCountFakeWriter) GetPRState(_ context.Context, _, _ string, _ int) (scm.PRState, error) {
	return scm.PRState{Author: "tatara-bot"}, nil
}
func (f *closeCountFakeWriter) Comment(_ context.Context, _, _, _ string) error { return nil }

// TestWriteBackSelfImprove_CloseClears verifies finding 6: after a successful
// ClosePR the controller must clear WritebackPending immediately so a requeue
// does not re-call ClosePR and re-post the close comment.
func TestWriteBackSelfImprove_CloseClears(t *testing.T) {
	fw := &closeCountFakeWriter{}
	r := &TaskReconciler{
		Client:  k8sClient,
		Scheme:  k8sClient.Scheme(),
		Metrics: obs.NewOperatorMetrics(prometheus.NewRegistry()),
	...
	task := seedWritebackKindTask(t, "si-close-clear", "scc-proj", "scc-repo", "scc-scm",
		tatarav1alpha1.TaskSpec{
			Goal: "self-improve",
			Kind: "selfImprove",
			Source: &tatarav1alpha1.TaskSource{
				Provider: "github", IsPR: true, Number: 99,
			},
		},
		&tatarav1alpha1.ScmSpec{
			Provider: "github", Owner: "o", BotLogin: "tatara-bot",
		})
	task.Status.PROutcome = &tatarav1alpha1.PROutcome{Action: "close", Reason: "exhausted retries"}
	require.NoError(t, k8sClient.Status().Update(context.Background(), task))

	// First reconcile: ClosePR succeeds, WritebackPending must be cleared.
	_, err := reconcileWriteback(t, r, task.Name)
	require.NoError(t, err)
	...
	require.Equal(t, metav1.ConditionFalse, cond.Status,
		"WritebackPending must be cleared after successful ClosePR")

	// Second reconcile: WritebackPending is already False -> doWriteBack is not
	// called again; ClosePR must NOT be called a second time.
	_, err = reconcileWriteback(t, r, task.Name)
	require.NoError(t, err)
	...
	calls := fw.closeCalls
	fw.mu.Unlock()
	require.Equal(t, 1, calls, "ClosePR must be called exactly once; requeue must not re-post close")
}
