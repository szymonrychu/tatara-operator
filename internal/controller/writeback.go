...
func (r *TaskReconciler) writeBackOpenChange(ctx context.Context, task *tatarav1alpha1.Task) (ctrl.Result, error) {
	...
	if len(inScopeNoBranch) > 0 && task.Spec.Source != nil && task.Spec.Source.IssueRef != "" {
		...
		_, werr := r.gatedComment(ctx, &proj, &primaryRepo, writer, token, provider,
			task.Spec.Source.Number, task.Spec.Source.IsPR, task.Spec.Source.AuthorLogin,
			task.Spec.Source.IssueRef, warnBody)
		if werr != nil {
	...
	if len(prURLs) == 0 {
		...
		if commented {
			_, cerr := r.gatedComment(ctx, &proj, &primaryRepo, writer, token, provider,
				task.Spec.Source.Number, task.Spec.Source.IsPR, task.Spec.Source.AuthorLogin,
				task.Spec.Source.IssueRef, task.Status.ResultSummary)
			if cerr != nil {
	...
	if task.Spec.Source != nil && task.Spec.Source.IssueRef != "" {
		...
		_, cerr := r.gatedComment(ctx, &proj, &primaryRepo, writer, token, provider,
			task.Spec.Source.Number, task.Spec.Source.IsPR, task.Spec.Source.AuthorLogin,
			task.Spec.Source.IssueRef, commentBody)
		if cerr != nil {
