...
func newSCMMetrics(reg prometheus.Registerer) *scmMetrics {
	m := &scmMetrics{
		scmWritesTotal: prometheus.NewCounterVec(prometheus.CounterOpts{
			...
			Help: "Total SCM operations by provider, verb, kind (read|write), and result (ok|error|blocked|gone|suppressed_last_word|suppressed_bot_mr|suppressed_closed|suppressed_duplicate).",
